					 ...
					 struct net_device *dev)
{
	...
	struct ieee80211_sub_if_data *tmp_sdata, *sdata, *first_sdata;//addx add
	...
	list_for_each_entry_rcu(tmp_sdata, &local->interfaces, list) {
		...
		if (tmp_sdata->vif.type == NL80211_IFTYPE_MONITOR ||
			...
			continue;
		if (first_sdata == NULL)// addx add
			first_sdata = tmp_sdata;// addx add
	...
	if (!cfg80211_reg_can_beacon(local->hw.wiphy, chandef,
				     sdata->vif.type)){// addx change
		printk(">>> %s: cfg80211_reg_can_beacon failed\n", __FUNCTION__);//addx add
		goto fail_rcu;
					 }// addx add
#endif
